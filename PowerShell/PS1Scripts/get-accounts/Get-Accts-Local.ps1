...
Function List-Disable-LocalUserAccount{
    ...
    ForEach ($comp in $ComputerName){
        ...
        ForEach ($user in $user_accts)
        {
            ...
            {
                LogWrite "$($user.Name)`tFlags: $($flags)" $true
            }
        }
    }
...
Function Find-InActive-LocalUserAccount{
    ...
    ForEach ($comp in $ComputerName){
        ...
        ForEach ($user in $user_accts)
        {
            Try
            {
                ...
                if (((New-TimeSpan -Start $user_inact -End $DateCheck).days -ge $TimeSpan.days ) -And "$flags" -notlike "*ACCOUNTDISABLE*")
                {
                    LogWrite "ComputerName: $($comp)`tUserName: $($user.Name) is inactive past the time span ($($TimeSpan)) and not disabled" $true
    ...
    if ( $Found -gt 0 )
    {
        LogWrite "FAILURE! Accounts were found to be inactive and not disabled! Verify this finding!" $true
        $Summary[1]++
    }
    else
    {
...
Function Find-Expired-LocalUserAccount{
    ...
    ForEach ($comp in $ComputerName){
        ...
        ForEach ($user in $user_accts)
        {
            Try
            {
                ...
                if (($DateCheck -gt $user_expr) -And "$flags" -notlike "*ACCOUNTDISABLE*")
                {
                    LogWrite "ComputerName: $($comp)`tUserName: $($user.Name) is expired and not disabled" $true
    ...
    if ( $Found -gt 0 )
    {
        LogWrite "FAILURE! Accounts were found to be expired and not disabled! Verify this finding!" $true
        $Summary[1]++
    }
    else
    {
